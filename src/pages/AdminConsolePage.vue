...
<template>
  <div class="page admin-console-page">
    ...
    <section class="admin-banner panel"><span class="admin-banner-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 20 6v5c0 5-3.5 8.3-8 10-4.5-1.7-8-5-8-10V6z"/><path d="m9 12 2 2 4-4"/></svg></span><div><span class="eyebrow">Full demo privileges</span><h2>Every workflow boundary is visible</h2><p>Use this view to explain access, accountability, and operational health to stakeholders.</p></div><StatusPill label="Admin access" tone="good" /></section>
    ...
    <div class="admin-console-grid"><section class="panel access-matrix-panel"><div class="panel-heading"><div><span class="eyebrow">Role boundaries</span><h2>Persona access matrix</h2></div><span class="muted-label">Demo policy</span></div><div class="access-matrix"><div v-for="row in accessRows" :key="row.label" class="access-row"><span class="access-row-icon" :class="`tone-${row.tone}`"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 4h14v16H5z"/><path d="M8 8h8M8 12h8M8 16h5"/></svg></span><span><strong>{{ row.label }}</strong><small>{{ row.scope }}</small></span><StatusPill :label="row.pages" tone="neutral" /><span aria-hidden="true">→</span></div></div><div class="panel-footnote"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 10v6M12 7.5v.5"/></svg><span>Admin visibility is broad, but acceptance, review, and release decisions still show their named authority.</span></div></section>
    ...
    <section class="panel admin-activity-panel"><div class="panel-heading"><div><span class="eyebrow">Operations</span><h2>Recent activity</h2></div><button type="button" class="text-button" @click="navigate('integration')">Open integration health <span aria-hidden="true">→</span></button></div><div class="admin-activity-list"><div v-for="item in activity" :key="item.time + item.title" class="admin-activity-row"><span class="activity-dot" :class="`tone-${item.tone}`"></span><span class="activity-time">{{ item.time }}</span><span><strong>{{ item.title }}</strong><small>{{ item.detail }}</small></span></div></div></section>

    <section class="panel admin-shortcuts-panel"><div class="panel-heading"><div><span class="eyebrow">All workflow pages</span><h2>Jump to a control</h2></div></div><div class="admin-shortcut-grid"><button v-for="item in navItems" :key="item.key" type="button" class="admin-shortcut" @click="navigate(item.key)"><span>{{ item.label }}</span><span aria-hidden="true">→</span></button></div></section>
  </div>
</template>
