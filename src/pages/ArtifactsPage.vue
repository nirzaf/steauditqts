...
const visibleDocuments = computed(() => v5Documents.filter((item) => {
  ...
  const matchesFilter = filter.value === 'All outputs' || (filter.value === 'Ready' && ['FINALIZED', 'SIGNED', 'ACCEPTED', 'VALIDATED', 'RECEIVED', 'ISSUED', 'SUBMITTED', 'FROZEN'].some((state) => item.status.includes(state))) || (filter.value === 'Needs action' && !['FINALIZED', 'SIGNED', 'ACCEPTED', 'VALIDATED', 'RECEIVED', 'ISSUED', 'SUBMITTED', 'FROZEN'].some((state) => item.status.includes(state)))
...
const linkedRecord = computed(() => {
  ...
  const map = {
    ...
    'DOC-22': { id: `REPORT-${engagement.value?.clientId || '0018'}-${engagement.value?.period || '2026'}`, version: scopedCandidate.value?.revision ? `candidate revision ${scopedCandidate.value.revision}` : 'candidate package', state: clientPackagePublished.value ? 'DELIVERED_SIMULATION' : 'PENDING RELEASE', actor: 'ACT-PARTNER' },
    'DOC-23': { id: `FS-FINAL-${engagement.value?.clientId || '0018'}-${engagement.value?.period || '2026'}`, version: scopedCandidate.value?.revision ? `matched revision ${scopedCandidate.value.revision}` : 'matched candidate', state: clientPackagePublished.value ? 'DELIVERED_SIMULATION' : 'PENDING RELEASE', actor: 'ACT-PARTNER' },
  ...
  return map[item.id] || { id: item.id, version: 'synthetic fixture', state: item.status, actor: null }
})
const artifactPreview = computed(() => {
  const item = selected.value
  const record = linkedRecord.value
  return `STE AUDITFLOW · SYNTHETIC ARTIFACT\n${item?.title || 'Workflow output'}\n\nEngagement: ${engagement.value?.id || 'NO-SCOPE'}\nClient / period: ${engagement.value?.clientId || '—'} · ${engagement.value?.period || '—'}\nRecord: ${record.id}\nVersion: ${record.version}\nState: ${record.state}\nOwner: ${item?.owner || '—'}\n\nThis watermarked preview explains the record handoff. It is not a signed report, invoice, legal record or production evidence.`
})
...
<template>
  <div class="page artifacts-page">
    <PageHeader eyebrow="Named outputs · document center" title="Inspect the 26 workflow artifacts" description="Each output is tied to a synthetic record, version, owner, state, and destination. Use this page to explain what the portal stores at every handoff." />
