...
const operations = computed(() => scenario.operations.map((operation) => ({
  ...
  system: operation.type === 'UPLOAD_TRANSFER' ? 'Portal → SharePoint' : 'Graph + SharePoint',
  started: 'Latest check',
  duration: '—',
...
const toast = ref('')
const lastRun = computed(() => operations.value[0]?.id ? 'Latest connection check' : 'Not run')
...
function applyProviderSimulation() {
  ...
  toast.value = result.outcome === 'COMMITTED' ? `Provider setting updated to ${result.data.connected ? 'connected' : 'not connected'} · next outcome ${result.data.nextFault}.` : `${result.outcome}: ${result.code} — ${result.message}`
  window.setTimeout(() => { toast.value = '' }, 3500)
}
...
function resumeBackup() {
  ...
  toast.value = result.outcome === 'COMMITTED' ? 'Recovery resumed; outward effects remain disabled.' : `${result.outcome}: ${result.code} — ${result.message}`
  window.setTimeout(() => { toast.value = '' }, 4000)
}
</script>
...
<template>
  <div class="page">
    ...
    <section class="panel integration-control-panel">
    ...
    <section class="health-grid">
      <article class="health-card warn"><div class="health-card-top"><span class="health-icon"><Icon name="link" :size="18" /></span><StatusPill label="Not connected" tone="warn" /></div><h2>Microsoft Graph</h2><p>Selected-resource permissions are reviewed before a connection is enabled.</p><strong>Provider state: {{ providerConnected ? 'connected' : 'not connected' }}</strong></article>
      <article class="health-card warn"><div class="health-card-top"><span class="health-icon"><Icon name="folder" :size="18" /></span><StatusPill label="Not connected" tone="warn" /></div><h2>SharePoint</h2><p>Repository binding and version reconciliation are tracked for each engagement.</p><strong>Latest check: {{ lastRun }}</strong></article>
      <article class="health-card warn"><div class="health-card-top"><span class="health-icon"><Icon name="warning" :size="18" /></span><StatusPill label="Manual verification" tone="warn" /></div><h2>Purview</h2><p>Retention label observation is awaiting an explicit records-admin proof.</p><strong>Protection observation pending</strong></article>
      <article class="health-card warn"><div class="health-card-top"><span class="health-icon"><Icon name="key" :size="18" /></span><StatusPill label="Role-based access" tone="warn" /></div><h2>Entra ID</h2><p>Role access remains separate from professional approval authority.</p><strong>Identity connection pending</strong></article>
    </section>
    ...
    <section class="panel"><div class="panel-heading"><div><span class="eyebrow">Recovery posture</span><h2>Environment boundaries</h2></div><StatusPill label="Staging and production separated" tone="good" /></div><div class="environment-grid"><div><span class="environment-label">Development</span><strong>Practice records</strong><small>Test Graph app · no production identities</small></div><div><span class="environment-label">Staging</span><strong>Production-like, sanitized</strong><small>Separate secrets, site and SharePoint storage</small></div><div><span class="environment-label">Production</span><strong>Restricted deployment rights</strong><small>Backups, monitoring and administrator evidence</small></div></div></section>

    <section class="integration-recovery-grid">
      <article class="panel recovery-control-panel">
        <div class="panel-heading"><div><span class="eyebrow">P20 recovery control</span><h2>Backup, quarantine, reconcile</h2></div><StatusPill :label="recovery.state === 'RESUMED_SIMULATION' ? 'READY' : recovery.state" :tone="recovery.state === 'RESUMED_SIMULATION' ? 'good' : recovery.state === 'QUARANTINED' ? 'danger' : 'warn'" /></div>
        <dl class="detail-list recovery-detail-list"><div><dt>Active epoch</dt><dd>{{ recovery.activeEpoch }}</dd></div><div><dt>Backup</dt><dd>{{ recovery.backup?.id || 'Not captured' }}</dd></div><div><dt>Independent checkpoint</dt><dd>{{ recovery.externalCheckpointIndependent ? 'Required and separate' : 'Not configured' }}</dd></div><div><dt>Outward effects</dt><dd>{{ recovery.outwardEffectsEnabled ? 'Enabled' : 'Disabled' }}</dd></div></dl>
        <div class="button-row recovery-actions"><button type="button" class="button secondary" :disabled="!canSystemOperate" @click="captureBackup">Capture backup</button><button type="button" class="button secondary" :disabled="!canSystemOperate || !recovery.backup" @click="restoreBackup">Restore + quarantine</button><button type="button" class="button secondary" :disabled="!canSystemOperate || recovery.state !== 'QUARANTINED'" @click="reconcileBackup">Reconcile checkpoint</button><button type="button" class="button primary" :disabled="!canSystemOperate || recovery.state !== 'RECONCILED'" @click="resumeBackup">Resume checks</button></div>
        <p class="recovery-note"><Icon name="lock" :size="15" /> Independent checkpoint evidence and explicit authorization are required before outward effects are enabled.</p>
      </article>
      <article class="panel recovery-evidence-panel">
        <div class="panel-heading"><div><span class="eyebrow">Recovery evidence</span><h2>What is preserved</h2></div><span class="muted-label">Append-only activity log</span></div>
        <ul class="check-list"><li><span class="list-icon good"><Icon name="database" :size="14" /></span><span><strong>Backup scope is pinned</strong><small>{{ recovery.backup?.candidateIds?.length || 0 }} release candidate(s) and source revision {{ recovery.backup?.sourceRevision || '—' }}</small></span></li><li><span class="list-icon" :class="recovery.reconciliation ? 'good' : 'warn'"><Icon :name="recovery.reconciliation ? 'check' : 'clock'" :size="14" /></span><span><strong>Independent checkpoint</strong><small>{{ recovery.reconciliation?.checkpointId || 'Pending reconciliation' }}</small></span></li><li><span class="list-icon danger"><Icon name="shield" :size="14" /></span><span><strong>Outward effects fenced</strong><small>Recovery and retry actions stay controlled until all checks are complete.</small></span></li></ul>
      </article>
    </section>

    <section class="integration-provider-grid">
      <article class="panel integration-identities-panel">
        <div class="panel-heading"><div><span class="eyebrow">Credential separation</span><h2>Who is allowed to do what</h2></div><span class="muted-label">V5 execution groups</span></div>
