...
const releaseStatus = computed(() => candidate.value?.archiveState === 'VERIFIED' ? 'Archive verified' : candidate.value?.deliveryState === 'DELIVERED_SIMULATION' ? 'Delivered' : candidate.value?.releaseEventId ? 'Release event committed' : blockers.value.length ? 'Blocked' : 'Ready to advance')
...
const legalHolds = computed(() => (scenario.legalHolds || [])
  .filter((item) => item.engagementId === selectedEngagement.value?.id)
  .map((item) => ({ ...item, reason: String(item.reason || '').replace(/\bsynthetic\s*/gi, '').replace(/\s{2,}/g, ' ').trim() })))
...
const safetyChecks = computed(() => [
  ...
  { label: 'Protection + checkpoint', value: candidate.value.checkpointId || 'Pending', detail: candidate.value.protection === 'VERIFIED_SIMULATION' ? 'Protection observed' : 'Protection or checkpoint still required', icon: 'shield', state: candidate.value.checkpointId ? 'Verified' : 'Blocks release', tone: candidate.value.checkpointId ? 'good' : 'danger' },
])
...
function createReleaseCandidate() {
  inspectedIndex.value = 0
  toast.value = candidate.value.id === 'NO-CANDIDATE'
    ? 'No release candidate exists in this engagement scope. Complete the review and completion gates before creating one in the target Frappe workflow.'
    : `${candidate.value.id} opened for inspection. Rail clicks only inspect; guarded commands are required to advance.`
