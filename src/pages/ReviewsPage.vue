  ...
  <div class="page">
    ...
    <section v-if="sharedDemoEnabled" class="panel shared-completion-panel" aria-labelledby="shared-completion-title">
      ...
      <div class="split-grid">
        <form class="shared-completion-form" @submit.prevent="submitSharedManager">
          ...
          <label>Decision<select v-model="managerDecision" name="manager-decision"><option>RECOMMEND_COMPLETE</option><option>RETURN_TO_TEAM</option><option>HOLD</option></select></label>
          <label>Rationale<input v-model="managerRationale" name="manager-rationale" type="text" autocomplete="off" placeholder="Basis for the decision (required)…" /></label>
          ...
          <small v-if="!canSharedManager">Available to the Audit Manager role.</small>
        </form>
        <form class="shared-completion-form" @submit.prevent="submitSharedPartner">
          ...
          <label>Decision<select v-model="partnerDecision" name="partner-decision"><option>APPROVE_FOR_OPINION</option><option>RETURN_TO_MANAGER</option><option>HOLD</option></select></label>
          <label>Rationale<input v-model="partnerRationale" name="partner-rationale" type="text" autocomplete="off" placeholder="Basis for the review (required)…" /></label>
          ...
          <small v-if="!canSharedPartner">Available to the Partner role.</small>
        </form>
        <form class="shared-completion-form" @submit.prevent="submitSharedEqr">
          ...
          <label>Decision<select v-model="eqrDecision" name="eqr-decision"><option>APPROVE</option><option>RETURN</option><option>HOLD</option></select></label>
          <label>Candidate<select v-model="eqrCandidate" name="eqr-candidate" aria-label="Published Draft FS candidate"><option v-for="item in eqrCandidates" :key="item.version" :value="item.version">{{ item.label }}</option></select></label>
          <small v-if="!eqrCandidates.length">No published Draft FS candidate yet — the partner must publish one first.</small>
          <label>Note<input v-model="eqrNote" name="eqr-note" type="text" autocomplete="off" placeholder="Required when returning or holding…" /></label>
          ...
          <small v-if="!canSharedEqr">Available to the EQR role.</small>
        </form>
      </div>
    ...
    <section class="panel completion-recommendation-panel">
      ...
      <div class="completion-recommendation-body">
        ...
        <div class="completion-recommendation-form"><label>Recommendation<select v-model="completionDecision" name="completion-recommendation"><option value="RECOMMEND">Recommend completion</option><option value="HOLD">Keep on hold</option><option value="RETURN_FOR_CORRECTION">Return for correction</option></select></label><label>Rationale<textarea v-model="completionRationale" name="completion-rationale" rows="4" maxlength="500" placeholder="Summarize the review performed and remaining partner/EQR blockers…"></textarea></label><button v-if="canRecommendCompletion" type="button" class="button primary" :disabled="completionWorking || completionRationale.trim().length < 8" @click="recordCompletion">{{ completionWorking ? 'Recording…' : 'Record recommendation' }} <Icon name="check-circle" :size="16" /></button><p v-else class="form-safety-note"><Icon name="lock" :size="15" />Only the assigned audit manager can record this handoff.</p></div>
      </div>
