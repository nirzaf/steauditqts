...
const toast = ref('')
// P0.1 — deep-link to an exact review point / approval record.
const reviewSelection = useRecordSelection({ getIds: () => [...points.value.map((p) => p.id), ...approvalItems.value.map((a) => a.targetId || a.id)], initial: null })
...
watch(() => reviewSelection.targetedId.value, (id) => {
  if (!id) return
  // Ensure a deep-linked review point is visible even under My queue / Blocking.
  if (points.value.some((p) => p.id === id)) activeFilter.value = 'All points'
})
  ...
  <div class="page">
    ...
    <section v-if="sharedDemoEnabled" class="panel approval-center-panel" aria-labelledby="approval-center-title">
      ...
      <ul v-else class="approval-center-list">
        <li v-for="item in approvalItems" :key="`${item.kind}-${item.id}`" class="approval-center-row" :data-record-id="item.targetId || item.id" :class="{ 'record-target': reviewSelection.isTarget(item.targetId || item.id) }">
