...
<template>
  <div class="page accountant-client-page">
    ...
    <section class="panel read-only-banner"><span class="read-only-icon"><Icon name="lock" :size="20" /></span><div><span class="eyebrow">Accountant access</span><h2>Read-only client facts</h2><p>{{ source === 'd1' ? 'Showing the latest profile submitted through the shared synthetic record.' : syncState === 'LOCAL_ONLY_FALLBACK' ? 'Showing a saved local draft while the shared API is unavailable; it is not synchronized.' : 'Showing browser-local synthetic state for this walkthrough.' }}</p></div><StatusPill label="Cannot edit here" tone="neutral" /></section>
