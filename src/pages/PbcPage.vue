...
const selectedRequest = computed(() => requests.value.find((item) => item.id === selectedId.value) || requests.value[0] || { id: '—', title: 'No scoped requests', status: 'Not available', tone: 'neutral', area: '—', due: '—', owner: '—', files: 0, progress: 0, note: 'This selected engagement has no PBC request yet.' })
...
function selectRequest(id) { recordSelection.select(id) }
