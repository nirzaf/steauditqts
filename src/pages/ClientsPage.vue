...
<template>
  <div class="page">
    ...
    <section class="panel client-register-panel">
      ...
      <div class="toolbar"><label class="search-field"><Icon name="search" :size="17" /><span class="sr-only">Search clients</span><input v-model="search" name="client-search" type="search" autocomplete="off" spellcheck="false" placeholder="Search by name, ID or service…" /></label><div class="filter-row" aria-label="Client filters"><button v-for="item in filters" :key="item" type="button" :class="{ active: filter === item }" @click="filter = item">{{ item }}</button></div></div>
