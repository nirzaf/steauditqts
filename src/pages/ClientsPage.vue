  ...
  <div class="page">
    ...
    <section class="split-grid">
      <article class="panel assessment-card">
        ...
        <div class="hold-list"><div><span class="hold-icon danger">!</span><span><strong>UBO verification evidence expired</strong><small>Blocks commencement until a current registry extract is verified.</small></span><StatusPill label="Blocking" tone="danger" /></div><div><span class="hold-icon warn">!</span><span><strong>Accounting system changed</strong><small>Update the data-risk assessment and pin the new source bridge.</small></span><StatusPill label="Specialist" tone="warn" /></div></div>
        <div class="card-footer"><span>Partner decision remains human-owned</span><button type="button" class="button secondary" @click="openEngagement">Open engagement <span aria-hidden="true">→</span></button></div>
      </article>
      <article class="panel selected-client" v-if="selected">
        <div class="panel-heading"><div><span class="eyebrow">Client profile</span><h2>{{ selected.name }}</h2></div><button type="button" class="icon-button" aria-label="Close client profile" @click="selected = null"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div>
        ...
        <div class="card-footer"><button type="button" class="button primary" @click="openEngagement">Open workspace</button></div>
      </article>
      <article class="panel empty-side" v-else><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5A2.5 2.5 0 0 1 6.5 3h11A2.5 2.5 0 0 1 20 5.5v13a2.5 2.5 0 0 1-2.5 2.5h-11A2.5 2.5 0 0 1 4 18.5v-13Z"/><path d="M8 8h8M8 12h5"/></svg><h3>Select a client to inspect the relationship</h3><p>Client facts, decisions, evidence and permissions remain scoped to the selected professional relationship.</p></article>
    </section>

    <div class="prototype-note"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 10v6M12 7.5v.5"/></svg><span><strong>Acceptance boundary</strong> A customer record, quotation, deposit or portal login never implies professional acceptance. The partner decision is stored separately.</span></div>
  </div>
</template>
