    ...
    <section v-if="sharedEnabled" class="shared-demo-action-layout">
      <article class="panel shared-action-panel">
        ...
        <form v-if="activeAction" class="shared-action-form" @submit.prevent="submitAction">
          ...
          <div v-if="activeAction.fields.length" class="form-grid compact-form-grid"><label v-for="[field, label, placeholder] in activeAction.fields" :key="field" :class="{ 'span-two': field === 'context' || field === 'description' || field === 'acceptanceCriteria' || field === 'rationale' || field === 'conclusion' || field === 'detail' || field === 'explanation' || field === 'note' || field === 'response' }">{{ label }}<textarea v-if="['context', 'description', 'acceptanceCriteria', 'rationale', 'conclusion', 'detail', 'explanation', 'note', 'response', 'comment'].includes(field)" v-model="form[field]" :name="`shared-${field}`" autocomplete="off" rows="2" :placeholder="placeholder" /><input v-else v-model="form[field]" :name="`shared-${field}`" type="text" autocomplete="off" :placeholder="placeholder" /></label></div>
