...
function chooseUpload(request) {
  uploadRequest.value = request; uploadFile.value = null; uploadFileName.value = ''; uploadStatus.value = ''; uploadStatusTone.value = 'neutral'; uploadOutcome.value = null
...
async function submitUpload() {
  ...
  if (isSharedInvitation.value) {
    ...
    uploadOutcome.value = { ...result, outcome: result.ok ? 'COMMITTED' : (result.outcome || 'REJECTED'), message: result.ok ? `${uploadFileName.value} is now available until ${result.expiresAt || result.expires_at || 'the expiry time'}.` : (result.error?.message || 'The file was not stored. Retry the same upload.') }
    if (result.ok) {
    ...
    uploadOutcome.value = { ...result, outcome: result.outcome || 'REJECTED', message: result.outcome === 'COMMITTED' ? `${uploadFileName.value} was received as ${result.data?.receiptId || 'a versioned receipt'}.` : `${result.outcome}: ${result.code} — ${result.message}` }
...
async function downloadReceipt(receipt) {
  ...
  try {
    ...
    uploadOutcome.value = { ok: true, outcome: 'COMMITTED', message: uploadStatus.value }
  } catch (error) { uploadStatus.value = error.code === 'FILE_EXPIRED' ? 'This evidence expired after 24 hours and can no longer be downloaded.' : error.message; uploadStatusTone.value = 'danger'; uploadOutcome.value = { ok: false, outcome: error.code === 'FILE_EXPIRED' ? 'REJECTED' : 'ERROR', message: uploadStatus.value } }
}
