...
const auditSelection = useRecordSelection({ getIds: () => [...risks.value.map((r) => r.id), ...liveWorkpapers.value.map((w) => w.id)], initial: null })
...
// P0.1 — consume ?record=: risks stay on Planning, workpapers switch to Fieldwork.
watch(() => auditSelection.targetedId.value, (id) => {
  if (!id) return
  if (risks.value.some((r) => r.id === id)) { selectedRiskId.value = id; activeTab.value = 'Planning' }
  else if (liveWorkpapers.value.some((w) => w.id === id)) { activeTab.value = 'Fieldwork' }
})
...
<template>
  <div class="page">
    ...
    <template v-else-if="activeTab === 'Fieldwork'">
      <section class="panel"><div class="panel-heading"><div><span class="eyebrow">Working papers</span><h2>Evidence-to-conclusion workspace</h2></div><span class="muted-label">{{ liveWorkpapers.length }} workpapers · exact snapshots</span></div><div class="workpaper-grid"><article v-for="workpaper in liveWorkpapers" :key="workpaper.id" class="workpaper-card" :data-record-id="workpaper.id" :class="{ 'record-target': auditSelection.isTarget(workpaper.id) }"><div class="workpaper-top"><span class="workpaper-id">{{ workpaper.id }}</span><StatusPill :label="workpaper.status" :tone="workpaper.tone" /></div><h3>{{ workpaper.title }}</h3><p>{{ workpaper.area }} · {{ workpaper.evidence }} evidence snapshots</p><div class="workpaper-stage"><span>{{ workpaper.stage }}</span><i><b :style="{ width: workpaper.status === 'Reviewed' ? '100%' : workpaper.status === 'In review' ? '68%' : workpaper.status === 'Open' ? '48%' : '18%' }"></b></i></div><div class="card-footer"><span>Reviewer · {{ workpaper.reviewer }}</span><button type="button" class="row-button" :disabled="!workpaper.revision" @click="submitSnapshot(workpaper)">{{ workpaper.status === 'Not submitted' ? 'Submit snapshot' : 'New revision' }} <Icon name="arrow-right" :size="15" /></button></div></article></div></section>
